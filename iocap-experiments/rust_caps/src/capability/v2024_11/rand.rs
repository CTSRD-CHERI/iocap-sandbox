...
pub enum Cap2024_11_EdgeCase {
    ...
    Cav1_TooBig_IndexZero_Cav2_TooBig_IndexZero_Valid,
    Cav1_TooBig_IndexZero_Cav2_TooBig_IndexNonZero_Invalid,
...
impl From<Cap2024_11_EdgeCase> for &'static core::ffi::CStr {
    fn from(value: Cap2024_11_EdgeCase) -> Self {
        match value {
            ...
            Cap2024_11_EdgeCase::Cav1_TooBig_IndexZero_Cav2_TooBig_IndexZero_Valid => c"Cap2024_11_EdgeCase::Cav1_TooBig_IndexZero_Cav2_TooBig_IndexZero_Valid",
            Cap2024_11_EdgeCase::Cav1_TooBig_IndexZero_Cav2_TooBig_IndexNonZero_Invalid => c"Cap2024_11_EdgeCase::Cav1_TooBig_IndexZero_Cav2_TooBig_IndexNonZero_Invalid",
...
const CAP2024_11_EDGE_CASES: [Cap2024_11_EdgeCase; 9] = [
    ...
    Cap2024_11_EdgeCase::Cav1_TooBig_IndexZero_Cav2_TooBig_IndexZero_Valid,
    Cap2024_11_EdgeCase::Cav1_TooBig_IndexZero_Cav2_TooBig_IndexNonZero_Invalid,
        ...
        match edge_case {
                ...
                let (range_x, range_y_minus_one) = {
                    ...
                    match cav1_pow2.checked_sub(MAX_CAV2_CUTDOWN) {
                        Some(_) => {
                            ...
                            assert!(first_range_val_outside_cav1 >= 1);
                            // if first_range_val_outside_cav1 == MAX_RANGE - 1 {
                            //     dbg_hex::dbg_hex!(initial_range_len, cav1_pow2);
                            // }

                            let range_x = 0;
                            let range_y_minus_one = ChooseRand::Between(first_range_val_outside_cav1, MAX_RANGE - 1).sample(rng);
            ...
            Cap2024_11_EdgeCase::Cav1_TooBig_IndexZero_Cav2_TooBig_IndexNonZero_Invalid => {
                ...
                let data = {
                    ...
                    //
                    // use initial_range_len >>
                    let (range_x, range_y_minus_one) = match cav1_pow2.checked_sub(MAX_CAV2_CUTDOWN) {
                        Some(_) => {
                            // the cav1 pow2 >= MAX_CAV2_CUTDOWN so all bits of range_x, range_y will be used
                            const MAX_RANGE: u16 = 1 << MAX_CAV2_CUTDOWN;
                            // TODO check bit indexes
                            ...
                            let range_x = ChooseRand::Between(1, first_range_val_outside_cav1.saturating_sub(1)).sample(rng);
                            let range_y_minus_one = ChooseRand::Between(first_range_val_outside_cav1, MAX_RANGE - 1).sample(rng);
...
mod test {
    ...
    use crate::{capability::{AddrRange, CapEncodeErr, CapPerms, v2024::{Cap2024, rand::{CapTestCase, RandCap}}, v2024_11::Cap2024_11}, crypto::NullSig};

    use dbg_hex::dbg_hex;

    const RAND_SEED: u64 = 0;

    fn test_cav_chain(initial_base: u64, initial_len: u128, perms: CapPerms, cav_range: AddrRange) -> Result<(), (CapEncodeErr, u8)> {
        let cap =
            Cap2024_11::<NullSig>::new_inexact(
                0,
                0,
                initial_base,
                initial_len,
        ...
        let cap =
            cap.refine_using_cav1_towards(
                None,
                cav_range,
            ).map_err(|e| (e, 1))?;
        let _cap =
            cap.refine_using_cav2_towards(
                None,
                cav_range,
            ).map_err(|e| (e, 2))?;
        Ok(())
    ...
    fn test_api_replicates(test: CapTestCase) {
        match test.expected {
            Ok((addr_range, perms)) => {
                // API test - should always be able to derive-towards

                // Start from 1<<54 - 1 at most, because the initial region can't encode anything higher
                // Only attempt to reduce to 1, not 0 - the caveat API don't officially support that, it's an edge case for a reason

                let initial_base = match addr_range.base() {
                    b if b >= (1u64 << 54) => (1u64 << 54) - 1,
                    b => b
                };
                let initial_len = match addr_range.top() - initial_base as u128 {
                    0 => 1,
                    l => l,
                };

                let cav_base = addr_range.base();
                let cav_len = match addr_range.len() {
                    0 => 1,
                    l => l,
                };
                let cav_range = AddrRange::base_len(cav_base, cav_len);

                match test_cav_chain(initial_base, initial_len, perms, cav_range) {
                    Ok(_) => {},
                    Err((e, level)) => {
                        dbg_hex!(Cap2024_11::<NullSig>::from_bits(test.sig, test.data).unwrap().cap_data);
                        dbg_hex!(addr_range);
                        dbg_hex!(initial_base, initial_len);
                        dbg_hex!(cav_range);

                        panic!("Just had a successful test case, should be able to derive from first principles, but got error {e} at level {level}.\n")
                    },
                }
            }
            Err(_) => {}
    ...
    fn generate_lots_of_caps() {
        ...
        for _ in 0..10_000 {
            let test = Cap2024_11::<NullSig>::rand_cap(&mut rng, None, None, None, None);
            test_api_replicates(test);
        }
    }
    ...
    fn generate_lots_of_edge_caps() {
        ...
        for edge_case in Cap2024_11::<NullSig>::enum_edge_cases() {
            for _ in 0..10_000 {
                let test = Cap2024_11::<NullSig>::rand_edge_case(&mut rng, None, None, *edge_case);
                test_api_replicates(test);
            }
        }
    }
