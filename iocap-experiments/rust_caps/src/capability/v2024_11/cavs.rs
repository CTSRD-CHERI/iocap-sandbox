...
use crate::{
    capability::{
        v2024_11::{MAX_INITIAL_LENGTH, MIN_INITIAL_LENGTH},
        AddrRange, CapDecodeErr, CapEncodeErr,
    },
    checked_bits::{const_shift_right, right_pad, BitInt, CheckAdd, CheckLeftShift, CheckSub},
};
...
struct PostCaveat2Range {
    ...
    start: BitInt<u64, 64>,
    // Cap2024_11 EDIT 2026-04
    // If cav1 represents a index=0 pow2 range that exceeds the size of the initial resource,
    // it is considered legal and the end of the range is clamped to the end of the initial resource.
    // If cav2 then represents a range_x=0 range that exceeds the size of the initial resource,
    // it is considered legal and the end of the range is clamped to the end of the initial resource.
    // This is equivalent to capping the length to the length of the initial resource, because the start of the range == start of initial resource
    clamp_length: Option<BitInt<u128, 65>>,
    // Cap2024_11 EDIT END 2026-04
}
...
fn apply_cav1(
    initial: &PartialEncodedInitialRange,
    index: Cav1Index,
    index_size_div: Cav1SizeDiv,
) -> Result<PostCaveat1Range, BadCaveatReason> {
    ...
    let old_elem_width_log_2 = CheckAdd::<u8, 6>::add(
        initial.elem_width_log2_minus_10,
        BitInt::<u8, 6>::constant(10),
    );
    ...
    let check_lhs: BitInt<u64, 43> = Shifter::shl(
        CheckAdd::<u16, 16>::add(index, BitInt::<u8, 1>::constant(1)),
        initial.log_max_count,
    )
    .shrink();
    ...
    let elem_width_log_2 = CheckSub::<u8, 7>::sub(
        CheckAdd::<u8, 7>::add(old_elem_width_log_2, initial.log_max_count),
        index_size_div,
    );
...
fn apply_cav2(
    cav1: &PostCaveat1Range,
    range_x: Cav2RangeInt,
    range_y_minus_one: Cav2RangeInt,
) -> Result<PostCaveat2Range, BadCaveatReason> {
    ...
    // Hypothesis: Cav2 does not need to be valid in any case where the cav2 range exceeds the initial range, so we can directly return Cav2BadRange in that case.
    // Is this true? Let's think. The only case where this matters was if cav1 reduced to pow2 that exceeds the initial range, which can happen for two reasons: (2026-04)
    //  - it was a null cav1 that shrunk permissions, in which case cav2 can only be trying to shrink space
    //  - it was a null cav1 trying to bring size_div down for the sake of cav2, which may not have shrunk permissions
    //      - which means cav2 needs to be able to represent the WHOLE addrspace
    // Conclusion: Cav2 DOES need to support exceeding cav1.clamp_length, and it should be treated as equivalent to cav1.clamp_length
    // 2026 EDIT END
    // Cap2024_11 EDIT END
    ...
    let (elem_count, elem_width_log_2, start_offset) =
        match CheckSub::<u8, 7>::sub_overflow(cav1.elem_width_log_2, BitInt::<u8, 7>::constant(14))
        {
            Some(e) => {
                ...
                if let Some(max_length) = cav1.clamp_length {
                    let top = CheckLeftShift::<u128, 65>::shl(
                        CheckAdd::<u16, 15>::add(range_y_minus_one, BitInt::<u8, 1>::constant(1)),
                        elem_width_log_2,
                    );
                    // If max_length < length, cav2 is OOB
                    if CheckSub::<u128, 65>::sub_overflow(max_length, top).is_none() {
                        // 2026_04 - cav2 can be OOB
                        if range_x.val() == 0 {
                            return Ok(PostCaveat2Range {
                                elem_count,
                                elem_width_log_2,
                                start: cav1.start,
            ...
            None => {
                ...
                let cav1_elem_width_log_2: BitInt<u8, 4> = BitInt::new_bound(
                    cav1.elem_width_log_2.val(),
                    cav1.elem_width_log_2.min()..=13,
                );
                ...
                let elem_count_shl =
                    CheckLeftShift::<u32, 28>::shl(elem_count, cav1_elem_width_log_2);
                ...
                if let Some(max_length) = cav1.clamp_length {
                    ...
                    if CheckSub::<u128, 65>::sub_overflow(max_length, length).is_none() {
                        // ...except in the case where x=0, just like the cav1 no-shrink case
                        // 2026_04
                        if range_x.val() == 0 {
                            return Ok(PostCaveat2Range {
                                elem_count: elem_count.expand(),
                                elem_width_log_2: BitInt::constant(0),
                                start: cav1.start,
                                clamp_length: cav1.clamp_length,
                            });
                        }
...
impl Caveats {
    pub fn apply_to(
        &self,
        initial: &PartialEncodedInitialRange,
    ) -> Result<AddrRange, BadCaveatReason> {
        match self {
...
mod test {
    ...
    use crate::{
        capability::{
            v2024_11::{
                cavs::{apply_cav1, apply_cav2, BadCaveatReason, ToStartLength},
                AddrRange,
            },
            CapDecodeErr,
        },
    ...
    impl PartialEncodedInitialRange {
        ...
        fn new_test(
            elem_width_log2_minus_10: u8,
            log_max_count: u8,
            elem_count: u32,
            base_addr_shr_10: u64,
        ) -> Self {
            Self {
    ...
    fn test_identity_cav1_on_max_size() -> Result<(), CapDecodeErr> {
        ...
        assert_eq_hex!(
            initial.to_addr_range()?,
            (0..(32 << 10)).into(),
            "initial range wasn't correct"
        );
        ...
        assert_eq_hex!(
            cav1.to_addr_range()?,
            (0..(32 << 10)).into(),
            "cav1 incorrectly shrunk the range"
        );
    ...
    fn test_big_unconstrained_direct() -> Result<(), CapDecodeErr> {
        ...
        assert_eq_hex!(
            cav2.to_addr_range()?,
            AddrRange::FULL_ADDR_SPACE,
            "cav2 incorrectly shrunk the range"
        );
        ...
        let cavs = Caveats::Two {
            index: BitInt::new(0),
            index_size_div: BitInt::new(0),
            range_x: BitInt::new(0),
            range_y_minus_one: BitInt::new(0x3FFF),
        };
    ...
    fn test_cav2_constraining_kilobyte() -> Result<(), CapDecodeErr> {
        ...
        let cavs = Caveats::Two {
            index: BitInt::new(0),
            index_size_div: BitInt::new(5),
            range_x: BitInt::new(0),
            range_y_minus_one: BitInt::new(0),
        };
        ...
        let cavs = Caveats::Two {
            ...
            index_size_div: BitInt::new(5),
            range_x: BitInt::new(0),
            range_y_minus_one: BitInt::new(1 << 4),
        };
    ...
    fn test_cav2_constraining_zero() -> Result<(), CapDecodeErr> {
        ...
        let cavs = Caveats::Two {
            ...
            index_size_div: BitInt::new(15),
            range_x: BitInt::new(0),
            range_y_minus_one: BitInt::new(0x0),
        };
        assert_eq_hex!(cavs.apply_to(&initial)?, (0..0).into());
    ...
    fn test_identity_cav1_blocks_oob_cav2() -> Result<(), CapDecodeErr> {
        ...
        let cavs = Caveats::Two {
            index: BitInt::new(0),
            index_size_div: BitInt::new(0),
            range_x: BitInt::new(1 << 9),
            range_y_minus_one: BitInt::new((1 << 9) + 1),
        };
