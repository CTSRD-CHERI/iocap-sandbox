...
//! The B|C layout has changed to that presented in https://docs.google.com/spreadsheets/d/1iCPURHf2zLnvSDNQREE8upmmwlQl5h2x7yhd1dHRd8w/edit?gid=722697455 - 10-to-64-bit length, with use of implicit one-bit 
//! The initial length encoding is identical to 2024_02, EXCEPT the base address is shortened by 10 bits, forcing it into a 54-bit address space.
...
impl<TSigner: Signaturer> Cap2024 for Cap2024_11<TSigner> {
    ...
    fn refine_using_cav1_towards(&self, new_perms: Option<CapPerms>, target: AddrRange) -> Result<Self, CapEncodeErr> {
        ...
        let top_rel_initial = base_rel_initial + (target.len() as u64);
        ...
        let top_excl_top16_ignoring_shiftout: u16 = (top_rel_initial >> min_pow2_of_cav1).try_into().unwrap();
        assert!(top_excl_top16_ignoring_shiftout <= (1 << 15));
        let top_had_shiftout = top_rel_initial & ((1u64 << min_pow2_of_cav1) - 1);
        let top_top16: u16 = if top_had_shiftout == 0 { top_excl_top16_ignoring_shiftout - 1 } else { top_excl_top16_ignoring_shiftout };
        assert!(top_top16 <= (1 << 15));

        let initial_len_top16 = (self.addr_range.len() >> min_pow2_of_cav1).try_into().unwrap();
        if top_top16 >= initial_len_top16 {
            // top can't be expressed at a fine enough granularity to not go beyond initial length.
            // This forces index and size_div == 0.
            // TODO future formats may want to remove this restriction!
            return self.with_cav1_size_div(new_perms, 0, 0)
        }
    ...
    fn refine_using_cav2_towards(&self, new_perms: Option<CapPerms>, target: AddrRange) -> Result<Self, CapEncodeErr> {
        ...
        let (range_x, range_y) = if cav1_pow2 >= 14 {
            ...
            // 2026-04 Subtract before add to avoid overflow
            let top_offset = target.base() - self.addr_range.base() + (target.len() as u64);
            ...
            let len_shifted = (target.len() >> slice_pow2) as u16 + if (target.len() as u64 & ((1u64 << slice_pow2) - 1)) > 0 { 1 } else { 0 };
            let range_x = if range_y >= len_shifted {
...
impl EncodedInitialRange {
    ...
    pub fn pack_base_and_length(r: &AddrRange) -> Result<(Self, AddrRange), CapEncodeErr> {
        let length = {
            let length = r.len();
    ...
    pub fn partial_decode(&self) -> PartialEncodedInitialRange {
