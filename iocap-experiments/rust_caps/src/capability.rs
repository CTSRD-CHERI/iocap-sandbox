...
impl AddrRange {
    ...
    pub const FULL_ADDR_SPACE: Self = Self {
        start: 0,
        end: 1 << 64,
    };
    ...
    pub fn start_end(start: u64, end: u128) -> Self {
        ...
        assert!(
            BitInt::<u128, 65>::check(end).is_some(),
            "Can't create an AddrRange with an `end` greater than 65 bits"
        );
        Self { start, end }
    }
...
impl core::fmt::LowerHex for AddrRange {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        write!(f, "[{:x}, {:x})", self.start, self.end)
    }
}
impl core::fmt::UpperHex for AddrRange {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        write!(f, "[{:X}, {:X})", self.start, self.end)
    }
}
...
pub enum CapEncodeErr {
    ...
    #[cfg_attr(
        feature = "std",
        error("Trying to apply a caveat that pushes the range out-of-bounds")
    )]
    InvalidCaveat,
