...
def decodeBaseAddr(quadrant, implicit_1, b_c) -> Tuple[int, int]:
    if quadrant == 0:
...
def decodeElemWidthLog2(encoded_elem_width) -> int:
    q = bits(encoded_elem_width, 4, 3)
    if q == 0:
...
def decodeLogMaxCount(quadrant, implicit_1) -> int:
    if quadrant == 0:
        return 5 + implicit_1
    elif quadrant == 1:
...
def unpack_shift_pair_long_bottom(x: int) -> Tuple[int, int]:
    return bits(x, 40, 0), bits(x, 54, 41)
...
def zero_cav(
    ...
    base, elem_count_minus_one = decodeBaseAddr(
        quadrant, implicit_1, b_c
    )  # bit splitting
...
def two_cav(
    ...
    if elem_width_log_2_sub_14 < 0:
        # 2026_04 - calculate initial_len in case we need to use it for identity_cav2
    ...
    if elem_width_log_2_sub_14 < 0:
        ...
        initial_length = shift_out
    else:
        end_offset = shift_out

        if range_x == 0:
            # 2026_04 compute initial_length early, we know what start_offset is
            shift_in = initial_elem_count
            shift_amt = initial_elem_width_log_2
        else:
    ...
    if elem_width_log_2_sub_14 < 0:
        # 2026_04
        shift_out = shift_in << shift_amt
        start_offset, length = unpack_shift_pair(shift_out)
        if range_x == 0:
            # Compute the original top
            # This uses the cav1 base BUT that's OK - we only use this value IF identity cav1, overflow doesn't even matter
            initial_top = base + initial_length
            base = base
        else:
        ...
        # Find cav2 start_offset or length
        shift_out = shift_in << shift_amt  # 65-bit shift of range_x
        if range_x == 0:
            # 2026_04 compute initial_length early, we know what start_offset is
            start_offset = 0
    ...
    if elem_width_log_2_sub_14 < 0:
        ...
        if range_x == 0:
            # 2026_04 compute initial_top early, we know what base is
            # This uses the cav1 base BUT that's OK - we only use this value IF identity cav1
            base = base
            initial_top = base + initial_length
        else:
            # Increment using start_offset from cav2
    ...
    if end_offset > initial_length:  # 64-bit cmp
        # 2026_04
        if identity_cav1 and range_x == 0:
            return base, initial_top
        raise InvalidCaveat()
...
def n_cav(
    ...
    if n_caveats == 0:
        return zero_cav(
            range_y_minus_one, range_x, index, index_size_div, encoded_elem_width, b_c
        )
    elif n_caveats == 1:
        return one_cav(
            range_y_minus_one, range_x, index, index_size_div, encoded_elem_width, b_c
...
def n_cav_together(
    ...
    elem_count = elem_count_minus_one + 1  # 5-bit add
    if n_caveats <= 1 and (
        range_x != 0 or range_y_minus_one != 0
    ):  # 18-bit zero comparison (4 ORs depth + 1 AND)
        raise UnexpectedCaveat()
    if n_caveats == 0 and (
        index_size_div != 0 or index != 0
    ):  # 19-bit zero comparison (4 ORs depth + 1 AND)
        raise UnexpectedCaveat()
    ...
    if n_caveats == 0:
        ...
        if n_caveats == 1:
            ...
            shift_out = (
                shift_in << shift_amt
            )  # 65-bit shift of either (range_x, elem_count) or range_y_minus_one+1
            if elem_width_log_2_sub_14 < 0:
                shift_in = pack_shift_pair(range_x, range_y_minus_one + 1 - range_x)
                shift_amt = elem_width_log_2
                elem_width_log_2 = 0  # x-bit assign
            ...
            if elem_width_log_2_sub_14 < 0:
                ...
                if range_x == 0:
                    # 2026_04 compute initial_length early, we know what base is
                    base = base
                    initial_top = base + initial_length
                else:
                    # Increment using start_offset from cav2
        ...
        if n_caveats == 1:
            ...
            if elem_width_log_2_sub_14 < 0:
                if range_x == 0:
                    ...
                    shl_to_shift = pack_shift_pair(
                        range_x, range_y_minus_one + 1 - range_x
                    )
                    shl_amt = elem_width_log_2
                    elem_width_log_2 = 0  # x-bit assign
            else:
        ...
        if n_caveats == 1:
            ...
            if elem_width_log_2_sub_14 < 0:
                ...
                if range_x == 0:
                    ...
                    add_op_65 = (AddSubOp.ADD, base, initial_length)
                    # Now compute length for the next one
                    shl_to_shift = pack_shift_pair(
                        range_x, range_y_minus_one + 1 - range_x
                    )
                    shl_amt = elem_width_log_2
            ...
            if elem_width_log_2_sub_14 < 0:
                # 2026_04
                if range_x == 0:
                    initial_top = res.added_65
                    base = base
                    start_offset, length = unpack_shift_pair(res.shifted)
                else:
                    base = res.added_65
                    if base >= (1 << 64):
                        raise InvalidCaveat()
...
def test_func_decoder(func, n_caveats, cap, test: "Test"):
    try:
        base, top = func(
            n_caveats,
            cap.range_y_minus_one,
            cap.range_x,
            cap.index,
            cap.index_size_div,
            cap.encoded_elem_width,
            cap.b_c,
        )
        if test.decode_err is not None:
            raise TestFail(
                f"{func.__name__} - Expected decode error {test.decode_err} but got None\n{cap}"
            )
        if (base, top) != (test.base, test.top):
            raise TestFail(
                f"{func.__name__} - ({base:x}, {top:x} != {test.base:x}, {test.top:x})\n{cap}"
            )
    except CapException as ex:
        if test.decode_err is None or type(test.decode_err) != type(ex):
            raise TestFail(
                f"{func.__name__} - Expected decode error {test.decode_err} but got {ex}\n{cap}"
            )
