...
int main(int argc, char **argv) {
    ...
    // #define FAIL do { res = EXIT_FAILURE; } while (0);
    #define FAIL return EXIT_FAILURE;
    while (std::getline(infile, line)) {
        fprintf(stderr, "\r%5lu ", line_no - 2);
        ...
        if (range_res == CCapResult_Success && !len_64 && (len <= UINT32_MAX) && (len > 0) && (perms != CCapPerms_ReadWrite)) {
            ...
            if (read_virtio_res == CCapResult_Success) {
                ...
                if (read_secret_id_res == CCapResult_Success) {
                    ...
                    } else if (init_virtio_cavs_exact_res != CCapResult_Encode_UnrepresentableBaseRange){
                        fprintf(stderr, "Failed to exactly reencode cap from virtio on line %ld %s\n", line_no, ccap_result_str(init_virtio_cavs_exact_res));
                        FAIL;
                    } else {
                        fprintf(stderr, "Non-fatal: reencoding line %ld with ccap2024_11_init_virtio_cavs_exact() produced an error %s, ignoring because this is likely due to index caveat being bad (thesis $5.5.4)\n", line_no, ccap_result_str(init_virtio_cavs_exact_res));
                    }
                } else {
        ...
        if (range_res == CCapResult_Success && !len_64 && len > 0) {
            ...
            CCapResult init_inexact_res = ccap2024_11_init_inexact(
                &reencoded_cap, &test.secret, base, len, 0, CCapPerms_ReadWrite, NULL, NULL, NULL
            );
            ...
            if (init_exact_cav_res == CCapResult_Success) {
            ...
            } else if (init_exact_cav_res != CCapResult_Encode_UnrepresentableBaseRange){
                ...
                fprintf(stderr, "Non-fatal: reencoding line %ld with ccap2024_11_init_cavs_exact() produced an error %s, ignoring because this is likely due to index caveat being bad (thesis $5.5.4)\n", line_no, ccap_result_str(init_exact_cav_res));
            }
        }
        ...
        switch (test.expected_result) {
            ...
            case CCapResult_Decode_InvalidSignature:
                // the perms_res is not correctly encoded for decode-fail cases
                // if (perms_res == CCapResult_Success) {
                //     if (perms != test.expected_perms) {
                //         fprintf(stderr, "Expected line %ld to decode perms %s, but it decoded %s\n", line_no, ccap_perms_str(test.expected_perms), ccap_perms_str(perms));
                //         FAIL;
